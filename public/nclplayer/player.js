(() => {
  ...
  function addLog(type, msg, subtitle = "") {
    const div = document.createElement("div");
    div.className = `log-entry ${type}`;
    const time = (/* @__PURE__ */ new Date()).toLocaleTimeString();
    let html = `<span class="log-time">[${time}]</span>`;
    if (subtitle) html += `<span class="log-topic">${subtitle}</span>`;
    html += `<span>${msg}</span>`;
    div.innerHTML = html;
  ...
  var CONFIG = {
    base_url: "/bundle",
    service_id: "tv-service-01",
    app_id: "app-video-01",
    // Empty by default: transport.js derives the standalone fallback
    // (ws://<pagehost>:7857) when this is absent. Integrated hosts (AOP) supply
    // the shared broker's WS URL here, including any path suffix (/mqtt).
    broker_ws_url: ""
  };
  var CONFIG_KEYS = ["base_url", "service_id", "app_id", "broker_ws_url"];
  ...
  async function loadConfig() {
    try {
      ...
      if (res.ok) {
        ...
        for (const key of CONFIG_KEYS) {
          if (fromServer[key] != null) CONFIG[key] = fromServer[key];
        }
      }
    } catch (e) {
      addLog("error", `Falha ao carregar ./config.json: ${e.message}. Usando defaults.`);
    }
    Object.assign(CONFIG, queryOverrides());
    addLog("system", `Config: base_url=${CONFIG.base_url} service_id=${CONFIG.service_id} app_id=${CONFIG.app_id}`);
  }
  function appTopicWildcard() {
    return `aop/${CONFIG.service_id}/${CONFIG.app_id}/doc/#`;
  }
...
`) && (e = e.replace(Kx, "%0A")), e.includes("\r") && (e = e.replace(Qx, "%0D")), e.includes("	") && (e = e.replace(Yx, "%09")), n.pathname = e, n;
  ...
  // src/transport.js
  function brokerURL() {
    if (CONFIG.broker_ws_url) return CONFIG.broker_ws_url;
    const host = window.location.hostname || "localhost";
    return `ws://${host}:7857`;
  }
  var client = null;
  function initMQTT(onCommand) {
    const url = brokerURL();
    addLog("system", `Broker: ${url}`);
    client = mqtt_esm_default.connect(url);
    client.on("connect", () => {
      updateStatus("Conectado", "connected");
      console.log("Conectado ao MQTT");
      const wildcard = appTopicWildcard();
      client.subscribe(wildcard, (err) => {
        if (!err) {
          addLog("system", `Inscrito em: ${wildcard}`);
        }
      });
    });
    client.on("reconnect", () => updateStatus("Reconectando...", "reconnecting"));
    client.on("close", () => updateStatus("Desconectado", "disconnected"));
    client.on("error", (err) => addLog("error", `Erro MQTT: ${err.message}`));
    client.on("message", (topic, message) => {
      try {
        const msgString = message.toString();
        const commandWrapper = JSON.parse(msgString);
        if (commandWrapper.action && commandWrapper.payload) {
          onCommand(topic, commandWrapper.action, commandWrapper.payload);
        } else {
          addLog("info", `Msg ignorada em ${topic}`);
        }
      } catch (e) {
  ...
  function publishEvent(mediaId, eventType) {
    let state = "UNKNOWN";
    if (eventType === "onEnd") state = "SLEEPING";
    const topic = `aop/${CONFIG.service_id}/${CONFIG.app_id}/doc/${mediaId}/presentationEvent/stateNotification`;
    const payload = {
      id: mediaId,
      state,
  ...
  function publishSelection(mediaId, key) {
    const topic = `aop/${CONFIG.service_id}/${CONFIG.app_id}/doc/${mediaId}/selectionEvent/stateNotification`;
    const payload = {
      id: mediaId,
      state: "OCCURRING",
      originalEvent: "onSelection",
      key
    };
    if (client) client.publish(topic, JSON.stringify(payload));
    addLog("event-out", `Sele\xE7\xE3o: ${mediaId} (${key})`);
  }
  ...
  // src/focus.js
  var focusables = /* @__PURE__ */ new Map();
  var focusedId = null;
  var selectedId = null;
  function registerFocusable(id, el2, focus) {
    focusables.set(id, { el: el2, focus });
    if (focusedId === null) {
      setFocus(lowestFocusIndexId());
    } else {
      renderFocus();
    }
  }
  ...
  function lowestFocusIndexId() {
    let bestId = null;
    let bestIdx = Infinity;
    for (const [id, { focus }] of focusables) {
      const idx = parseFloat(focus.focusIndex);
      if (!Number.isNaN(idx) && idx < bestIdx) {
        bestIdx = idx;
        bestId = id;
      }
    }
  ...
  function idByFocusIndex(focusIndex) {
    for (const [id, { focus }] of focusables) {
      if (focus.focusIndex === focusIndex) return id;
    }
    return null;
  }
  ...
  function moveFocus(direction) {
    ...
    const { focus } = focusables.get(focusedId);
    const target = focus["move" + direction];
    if (!target) return;
    const targetId = idByFocusIndex(target);
    if (targetId) setFocus(targetId);
  }
  function selectFocused(key) {
    if (focusedId === null) return;
    selectedId = focusedId;
    renderFocus();
    publishSelection(focusedId, key);
  }
  function renderFocus() {
    for (const [id, { el: el2, focus }] of focusables) {
      if (id === selectedId) {
        applyBorder(
          el2,
          focus.selBorderColor || FOCUS_DEFAULTS.selBorderColor,
          focus.focusBorderWidth,
          focus.focusBorderTransparency
        );
      } else if (id === focusedId) {
        applyBorder(
          el2,
          focus.focusBorderColor || FOCUS_DEFAULTS.focusBorderColor,
          focus.focusBorderWidth,
          focus.focusBorderTransparency
        );
      } else {
  ...
  function applyBorder(el2, color, width, transparency) {
    const { widthCss, offsetCss } = borderDims(width || FOCUS_DEFAULTS.focusBorderWidth);
    el2.style.outlineStyle = "solid";
    el2.style.outlineWidth = widthCss;
    el2.style.outlineOffset = offsetCss;
    el2.style.outlineColor = colorWithTransparency(
      color,
      transparency || FOCUS_DEFAULTS.focusBorderTransparency
    );
  }
  function clearBorder(el2) {
    el2.style.outlineStyle = "";
    el2.style.outlineWidth = "";
    el2.style.outlineOffset = "";
    el2.style.outlineColor = "";
  }
  function borderDims(width) {
    const m = String(width).match(/^(-?\d+(?:\.\d+)?)(\D*)$/);
    if (!m) return { widthCss: cssDim(width) || "3px", offsetCss: "0" };
    const n = parseFloat(m[1]);
    const unit = m[2] || "px";
    const magnitude = `${Math.abs(n)}${unit}`;
    return { widthCss: magnitude, offsetCss: n < 0 ? `-${magnitude}` : "0" };
  }
  function colorWithTransparency(color, transparency) {
    const s = String(transparency).trim();
    let t = parseFloat(s);
    if (Number.isNaN(t) || t <= 0) return color;
    if (!s.endsWith("%") && t <= 1) t *= 100;
    const opaquePct = Math.max(0, Math.min(100, 100 - t));
    return `color-mix(in srgb, ${color} ${opaquePct}%, transparent)`;
  }

  // src/render.js
  function handleCommand(topic, action, data) {
    addLog(`cmd-${action.toLowerCase()}`, `[${action}] ${data.id}${data.type ? ` (${data.type})` : ""}`, topic);
    switch (action) {
      case "START":
        startMedia(data);
        break;
      ...
      case "RESUME":
        ...
        break;
      default:
        addLog("info", `Comando desconhecido: ${action}`);
    }
  }
  function startMedia(data) {
    stopMedia(data.id);
    let el2;
    const url = mediaURL(data.src);
    if (data.type === "video") {
      el2 = document.createElement("video");
      el2.src = url;
      el2.autoplay = true;
      el2.controls = false;
      el2.removeAttribute("controls");
      el2.muted = false;
      el2.onended = () => {
        addLog("event-out", `V\xEDdeo terminou: ${data.id}`);
        publishEvent(data.id, "onEnd");
      };
      el2.onplay = () => addLog("event-out", `V\xEDdeo iniciou: ${data.id}`);
      ...
      el2.tabIndex = -1;
      el2.onended = () => {
        addLog("event-out", `\xC1udio terminou: ${data.id}`);
        publishEvent(data.id, "onEnd");
      };
      el2.onplay = () => addLog("event-out", `\xC1udio iniciou: ${data.id}`);
    } else if (data.type === "image") {
      el2 = document.createElement("img");
      el2.src = url;
      el2.onload = () => addLog("event-out", `Imagem carregada: ${data.id}`);
    } else if (data.type === "text") {
      el2 = document.createElement("div");
      el2.className = "ncl-text";
      fetch(url, { cache: "no-store" }).then((res) => res.ok ? res.text() : Promise.reject(new Error(res.status))).then((txt) => {
        el2.textContent = txt.trim();
      }).catch((e) => {
        el2.textContent = "";
        addLog("error", `Falha ao carregar texto ${data.id}: ${e.message}`);
      });
    }
    if (!el2) return;
    el2.id = data.id;
    el2.classList.add("ncl-media");
    applyRegion(el2, data.region);
    stage.appendChild(el2);
    if (data.focus && data.focus.focusIndex) {
      registerFocusable(data.id, el2, data.focus);
    }
  }
  function stopMedia(id) {
    ...
    if (el2) {
      if (typeof el2.pause === "function") el2.pause();
      el2.remove();
      addLog("info", `M\xEDdia removida: ${id}`);
    }
  }
  function pauseMedia(id) {
    const el2 = document.getElementById(id);
    if (el2 && typeof el2.pause === "function") {
      el2.pause();
      addLog("info", `M\xEDdia pausada: ${id}`);
    }
  }
  function resumeMedia(id) {
    const el2 = document.getElementById(id);
    if (el2 && typeof el2.play === "function") {
      el2.play();
      addLog("info", `M\xEDdia retomada: ${id}`);
    }
  }
  function applyRegion(el2, region) {
    if (!region) {
      el2.style.width = "100%";
      el2.style.height = "100%";
      return;
    }
    el2.style.left = cssDim(region.Left) || "0";
    el2.style.top = cssDim(region.Top) || "0";
    el2.style.width = cssDim(region.Width) || "100%";
    el2.style.height = cssDim(region.Height) || "100%";
    el2.style.zIndex = region.ZIndex || 0;
  }

  // src/main.js
  console.log("Player NCL Carregado. Aguardando intera\xE7\xE3o do usu\xE1rio...");
  overlay.addEventListener("click", async () => {
    overlay.classList.add("hidden");
    await loadConfig();
    addLog("system", "Sistema de Player Iniciado...");
    initMQTT(handleCommand);
  });
  var CURSOR_KEYS = {
    ArrowUp: "Up",
    ArrowDown: "Down",
  ...
  window.addEventListener("keydown", (ev) => {
    if (ev.key in CURSOR_KEYS) {
      ev.preventDefault();
      moveFocus(CURSOR_KEYS[ev.key]);
